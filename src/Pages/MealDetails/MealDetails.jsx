...
const MealDetails = () => {
    ...
    const handelReview = async (e) => {
        ...
        const reviewCount = mealDetail.review;
        const userEmail = users?.email;

        const reviewData = { review, mealId, mealTitle, like, reviewCount, userEmail };
        const reviewRes = await axiosSecure.post('/review', reviewData);
        const reviewCountRes = await axiosSecure.patch(`/reviewCount/${mealId}`);
        const reviewUpdate = await axiosSecure.patch(`/reviewReview/${mealId}`);
    ...
    const handelLikeCount = async () => {
        ...
        const reviewLike = await axiosSecure.patch(`/reviewLike/${mealDetail._id}`);
        refetch();
    }

    return (
        <div>
            <section className="bg-white dark:bg-gray-900 min-h-[560px]">
                <div className="container px-6 py-10 mx-auto">
                    ...
                    <div className="mt-3 lg:-mx-6 lg:flex lg:items-center">
                        ...
                        <div className="mt-2 lg:w-1/2 lg:mt-0 lg:mx-6 space-y-5">
                            <div className="flex items-center mt-2">
                                <img className="object-cover object-center w-10 h-10 rounded-full" src={users.photoURL} alt="" />
                            ...
                            <p className="mt-3 text-sm text-gray-500 dark:text-gray-300 md:text-sm">
                                <span className="font-semibold underline">Description:</span> {mealDetail.description}
                            </p>

                            <p className="text-white">
                                <span className="font-semibold underline">Ingredients:</span> {mealDetail.ingredients}
                            </p>
